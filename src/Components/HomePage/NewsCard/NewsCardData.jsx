export const NewsCardData = [
    {
        id: 1
        ,title: 'Welcome'
        ,image: 'MainLogoWhite.svg'
        ,text: 'At The Vibe Collective, we empower individuals on their spiritual journey by providing insightful guidance and transformative experiences rooted in the metaphysical realm. We are dedicated to offering exceptional support in the form of tarot readings, astrology readings, mediumship, remote reiki, intuitive counseling, and more. We help our clients gain profound insights into their lives.'
        ,imageLeftSide: true
    },
    {
        id: 2
        ,title: 'Our Approach'
        ,image: 'portrait-moonBeach.png'
        ,text: 'We believe in embracing and nurturing the highest self within each person. We strive to assist our clients in connecting with their inner wisdom and spiritual essence, empowering them to make conscious choices and live authentically. Through our services, we aim to inspire self-confidence, self-love, and a profound sense of purpose, guiding each individual to align with their truest, most authentic selves.'
        ,imageLeftSide: false
    },
    {
        id: 3
        ,title: 'The Path the Self-Discovery'
        ,image: 'portrait-girlsField.png'
        ,text: 'We are passionate about illuminating the path to self-discovery, spiritual enlightenment, and personal transformation. Join us on this transformative journey and let us help you unlock the secrets of the universe, embrace your highest self, and live a life filled with purpose, love, and abundance.'
        ,imageLeftSide: true
    },
    {
        id: 4
        ,title: 'Collective Open May 2024'
        ,image: 'portrait-moonBeach.png'
        ,text: 'We are so excited to VIBE with you! /nPlease Join our waitlist or newsletter to be one of the first to join the collective and have a reading with one of our amazing collective readers.'
        ,imageLeftSide: false
    },
    {
        id: 5
        ,title: 'Tarot Tales'
        ,image: 'portrait-womainField3.png'
        ,text: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Unde pariatur debitis sapiente animi cupiditate cumque ducimus tenetur aliquam excepturi incidunt blanditiis vel officia optio labore rerum, temporibus molestiae suscipit eveniet!'
        ,imageLeftSide: true
    },
]